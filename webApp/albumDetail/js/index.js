$(function () {
    ...
    AlbumApis.getAlbumContent(programId)
        .then(function (data) {
            console.log(data);
            // 发行时间
            data.album.publishTime = dateFormart("yyyy-MM-dd",new Date(data.album.publishTime));
            // 背景图片
            $(".main-bg>img").attr("src",data.album.picUrl);
            //共xx首
            $(".header-select>span").html("(共"+data.songs.length+"首)");
            // 专辑信息模板
