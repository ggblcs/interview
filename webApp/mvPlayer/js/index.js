$(function () {
    ...
    function getVideoDetail() {
        MVDetailApis.getMVDetail(ids[index])
            .then(function (data) {
                // 发行时间
                data.data.playTime = formartNum(data.data.playCount);
    ...
    function getVideoUrl(){
        MVDetailApis.getMVUrl(ids[index])
            .then(function (data) {
                // $video.remove();
                $(".swiper-slide-active>figure").prepend($video);

                $video.attr("src",data.data.url);
                $video.get(0).load();
                //
                // $video.get(0).load();
            })
    ...
    function getVideoInfo() {
        MVDetailApis.getMVInfo(ids[index])
            .then(function (data) {
                // 点赞数的数值处理
    ...
    // 获取评论详情
    function getComment(){
        MVDetailApis.getMVComment(ids[index])
            .then(function (data) {
                // 所有评论的数组名
    ...
    function getRelatedVideo(fn){
        MVDetailApis.getSimiMV(ids[index])
            .then(function (data) {

                for (let i = 0;i<data.mvs.length;i++){
                    ids.push(data.mvs[i].id);
                    bgPic.push(data.mvs[i].cover);
                }
                fn();
            })
            .catch(function (e) {
